...
export default function Home() {
  ...
  return (
    <div className="animate-in fade-in duration-700">
          ...
          <div className="flex-1 space-y-8">
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-4">
              ...
              <Button
                ...
                variant="outline"
                size="lg"
                className="rounded-full gap-2 text-base px-8 h-14 bg-brand-ivory/70 backdrop-blur-sm"
              >
      ...
      <section className="bg-brand-dark text-brand-ivory border-y border-brand-ivory/10">
        <div className="container mx-auto max-w-6xl px-4 py-20 md:py-24">

          <div className="max-w-3xl mb-14 md:mb-16">
            <p className="text-sm font-semibold tracking-[0.18em] uppercase text-brand-accent mb-5">
              {config.content.home.sections.find((s) => s.id === "concerns")?.eyebrow ||
                "Protection, without the jargon"}
            </p>

            <h2 className="text-4xl md:text-5xl lg:text-6xl font-serif leading-[1.05] tracking-tight text-balance text-brand-ivory">
              {config.content.home.sections.find((s) => s.id === "concerns")?.heading ||
                "Most people don't know what they're actually covered for."}
            </h2>

            <p className="mt-6 text-lg md:text-xl text-brand-ivory/80 max-w-2xl leading-relaxed">
          ...
          <div className="grid md:grid-cols-3 border-t border-brand-ivory/15">
            {homeContent.protection.principles.map((principle, index) => (
              <div
                key={principle.number}
                className={
                  index === 0
                    ? "pt-8 md:pr-10 md:border-r border-brand-ivory/15"
                    : index === 1
                      ? "pt-8 md:px-10 md:border-r border-brand-ivory/15"
                      : "pt-8 md:pl-10"
                }
              >
                ...
                <h3 className="mt-5 text-2xl md:text-3xl font-serif leading-tight text-brand-ivory">
                  {principle.title}
                </h3>

                <p className="mt-4 text-brand-ivory/75 leading-relaxed">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {homeContent.solutions.cards.map((card, index) => (
              <Link
                key={card.number}
                href={card.link}
                className="group block bg-card border border-border rounded-2xl overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:shadow-xl"
              >
                {/* Card image */}
                <div className="relative h-44 overflow-hidden">
                  <img
                    src={
                      index === 0
                        ? "/images/solutions/medical-card.jpeg"
                        : index === 1
                          ? "/images/solutions/hibah.jpeg"
                          : index === 2
                            ? "/images/solutions/critical-illness.jpeg"
                            : "/images/solutions/protection-review.jpeg"
                    }
                    alt=""
                  ...
                  <span className="absolute left-5 top-5 text-xs font-semibold tracking-widest text-brand-ivory drop-shadow-md">
                    {card.number}
                  </span>
                </div>
            ...
            <div className="relative">
              ...
              <div className="grid grid-cols-6 gap-5 relative">
                {homeContent.process.steps.map((step) => (
                  <div key={step.number}>
                    <div className="w-10 h-10 rounded-full bg-secondary text-secondary-foreground flex items-center justify-center font-semibold text-sm mb-6 relative z-10">
                      {step.number}
                    </div>
          ...
          <div className="md:hidden space-y-7">
            {homeContent.process.steps.map((step) => (
              <div key={step.number} className="flex gap-5">
                <div className="w-9 h-9 shrink-0 rounded-full bg-secondary text-secondary-foreground flex items-center justify-center font-semibold text-sm">
                  {step.number}
                </div>
        ...
        <div className="container mx-auto max-w-4xl px-4 text-center space-y-8">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4 pt-4">
            ...
            <Button
              ...
              variant="outline"
              size="lg"
              className="rounded-full gap-2 px-8 h-14 text-base bg-brand-ivory"
            >
              <a href={contactLinks.whatsapp}>
                <MessageCircle className="w-5 h-5" />
